...
1. **Interface dùng chung:** [IEventBus](file://Common/src/main/java/com/common/eventbus/IEventBus.java) định nghĩa phương thức `publish`.
2. **Cấu hình nạp Bean động:** [EventBusConfiguration](file://Common/src/main/java/com/common/eventbus/EventBusConfiguration.java) sử dụng `@ConditionalOnProperty` để quét và nạp Bean thực thi tương ứng dựa vào key `common.event-bus.type`.
