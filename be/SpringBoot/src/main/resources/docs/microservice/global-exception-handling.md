...
Tất cả các API khi xảy ra lỗi đều phản hồi dữ liệu theo cấu trúc của Record [WrapperApiResponse](file://Common/src/main/java/com/common/dto/response/WrapperApiResponse.java):
...
Khi viết logic nghiệp vụ, nếu phát hiện dữ liệu không hợp lệ hoặc vi phạm quy tắc hệ thống, nhà phát triển chỉ cần ném ra [BaseException](file://Common/src/main/java/com/common/exception/BaseException.java):
