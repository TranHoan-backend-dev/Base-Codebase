...
> [!IMPORTANT]
> Đối với SQL (JPA), [BaseRepository](file://Common/src/main/java/com/common/repository/sql/BaseRepository.java) hiện đã kế thừa thêm interface `JpaSpecificationExecutor<T>`. Điều này cho phép bạn gọi trực tiếp tất cả các phương thức tìm kiếm nâng cao sử dụng JPA Specification, ví dụ: `repository.findAll(Specification, Pageable)`.
