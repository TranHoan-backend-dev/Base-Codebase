...
Cấu hình Redis được đặt dưới khóa `spring.data.redis` trong file cấu hình [application.yaml](file://Common/src/main/resources/application.yaml). Để tối ưu hóa kết nối, hệ thống sử dụng thư viện **Lettuce** làm driver kết nối mặc định của Spring Boot.
...
Để giải quyết triệt để các vấn đề trên, lớp [RedisConfiguration](file://Common/src/main/java/com/common/config/redis/RedisConfiguration.java) định nghĩa cấu hình ghi đè như sau:
