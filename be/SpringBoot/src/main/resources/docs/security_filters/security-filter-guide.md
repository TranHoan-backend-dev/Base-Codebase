...
- **Request Body (JSON)**: Đối với các request `application.json`, bộ lọc bọc request bằng [CachedBodyHttpServletRequest](file://Common/src/main/java/com/common/filter/CachedBodyHttpServletRequest.java) để lưu tạm mảng byte của body vào bộ nhớ, cho phép đọc đi đọc lại nhiều lần mà không bị lỗi cạn luồng (`ServletInputStream`).
...
Hệ thống [SecurityConfiguration](file://Common/src/main/java/com/common/config/security/SecurityConfiguration.java) sẽ tự động nạp danh sách này để cấp quyền truy cập tự do (`permitAll()`).
