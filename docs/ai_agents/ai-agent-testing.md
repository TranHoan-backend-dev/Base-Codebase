...
### B. Thực thi Pipeline 6 Phase trên Dự án Backend (`be/SpringBoot`)

Chạy agent mặc định với dự án `be/SpringBoot` (dùng Maven):

```powershell
antigravity_env\Scripts\python config/agents/java_test_agent/run.py --path be/SpringBoot --build-tool maven
```
...
- **Chạy toàn bộ test (Tắt lọc Git)**:
  ...
  ```powershell
  antigravity_env\Scripts\python config/agents/java_test_agent/run.py --path be/SpringBoot --all-tests
  ```

- **Chạy duy nhất 1 file test cụ thể**:

  ```powershell
  antigravity_env\Scripts\python config/agents/java_test_agent/run.py --path be/SpringBoot --test-class UserServiceTest
  ```
...
```powershell
antigravity_env\Scripts\python config/agents/java_test_agent/run.py --path be/SpringBoot --auto-approve
```
...
## 💡 6. Tích hợp Agent Skill (`java-unit-test`) cho `be/SpringBoot`

Đã khởi tạo Skill tại `be/SpringBoot/.agents/skills/java-unit-test/SKILL.md`.

Khi bạn đưa ra câu lệnh như `"chạy unit test cho be/SpringBoot"` hoặc `"fix test backend"`, AI Agent sẽ tự động kích hoạt Skill này để thực thi script runner `run_java_test_agent.py`.
