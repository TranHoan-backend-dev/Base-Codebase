...
1. **Cấu hình cơ sở (Root Config):** Định nghĩa tại [playwright.config.base.ts](file://playwright.config.base.ts) chứa các thiết lập chung như:
...
2. **Cấu hình cụ thể từng dự án (Sub-repo Config):**
   - **Next.js:** [fe/nextjs-base/playwright.config.ts](file://fe/nextjs-base/playwright.config.ts)
   - **Nuxt.js:** [fe/nuxtjs-base/playwright.config.ts](file://fe/nuxtjs-base/playwright.config.ts)
