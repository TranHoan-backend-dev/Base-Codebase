...
- **Tái sử dụng (DRY):** Hạn chế lặp lại code. Trích xuất các logic dùng chung ra các file utilities hoặc helper services.
...
- **Tài liệu & Bình luận (Documentation & Comments):**
  ...
  - Đối với **Backend**, khi tạo bất cứ tính năng mới nào, bắt buộc phải có tài liệu đặc tả (specification) và lưu vào thư mục `resources/docs/.specify`.
...
5. **Kế hoạch Triển khai Cuối cùng:** Sau khi phương án được chọn, hãy viết một tài liệu Kế hoạch Kỹ thuật từng bước (bao gồm đường dẫn file, logic xử lý, dữ liệu mock).
...
7. **Thực thi (Execution):** CHỈ viết code sau khi kế hoạch đã được phê duyệt. Tuân theo các chuẩn thiết kế chung (ví dụ: HeroUI v3 Compound patterns).
8. **Đảm bảo Chất lượng (QA):** Chạy `pnpm lint` và `pnpm tsc` (hoặc các lệnh test/build backend tương đương). Xác minh logic phân quyền GBAC và tính tương thích SSR.
